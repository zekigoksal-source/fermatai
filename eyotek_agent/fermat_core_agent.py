        ...
        if complexity == "local" and self.router.is_local_available:
            ...
            try:
                ...
                if len(answer.strip()) < 30:
                    ...
                    logger.info("  [ESKALASYON] Ollama yaniti cok kisa, Claude'a geciliyor")
...
def _clean_ollama_format(answer: str) -> str:
    ...
    # # Baslik → *Baslik*
    answer = _re.sub(r'^#{1,6}\s*(.+?)$', r'*\1*', answer, flags=_re.MULTILINE)
    # Kod blogu (```) temizle
    answer = _re.sub(r'```[\w]*\n', '', answer)
    answer = answer.replace('```', '')
    # Markdown link [text](url) → text (url)
    answer = _re.sub(r'\[([^\]]+)\]\(([^)]+)\)', r'\1 (\2)', answer)
    # Yasak emoji kaldir
    for bad in ('😈','👻','💀','🖕','🤬','💩','🤡'):
        answer = answer.replace(bad, '')
    # HTML entity temizligi
    answer = answer.replace('&amp;', '&').replace('&lt;', '<').replace('&gt;', '>')
    # Cok satir bos satir (3+) → 2 satir
    answer = _re.sub(r'\n{3,}', '\n\n', answer)
    # Bullet point • veya * → -
    answer = _re.sub(r'^\s*[•·]\s+', '  - ', answer, flags=_re.MULTILINE)

    # ══════════════════════════════════════════════════════════════════
    # A+ GÖRSEL ENFORCER — Ollama cevabı Claude standardına ZORLA uydur
    # Neo talimatı: "standart dışı görsel görmek istemiyorum, çok net"
    # ══════════════════════════════════════════════════════════════════
    answer = answer.strip()
    if not answer or len(answer) < 10:
        return answer

    # Tek uzun paragraf → cümlelere böl (nokta sonrası satır kır)
    if '\n' not in answer and len(answer) > 120:
        answer = _re.sub(r'\.\s+', '.\n', answer)

    # KONTROLLER
    has_separator = '---' in answer or '━' in answer
    has_emoji = bool(_re.search(r'[\U0001f300-\U0001f9ff]', answer))
    has_bold = '*' in answer
    has_closing = bool(_re.search(r'[?🎯]', answer[-80:])) if len(answer) > 20 else True
    lines = answer.split('\n')
    first_line = lines[0].strip() if lines else ''

    # ── ADIM 1: Selamlama satırında emoji yoksa ekle ──
    if not _re.search(r'[\U0001f300-\U0001f9ff]', first_line):
        emoji_map = {'soru': '📊', 'sinav': '📝', 'fizik': '⚡', 'mat': '📐',
                     'kimya': '🧪', 'bio': '🧬', 'tarih': '📜', 'turkce': '📖',
                     'merhaba': '🌟', 'selam': '😊', 'iyi': '☀️'}
        picked = '💡'
        for k, v in emoji_map.items():
            if k in answer[:200].lower():
                picked = v
                break
        lines[0] = first_line + f' {picked}'

    # ── ADIM 2: Ayırıcı yoksa → selamlama sonrasına + sona ekle ──
    if not has_separator:
        # Selamlama (ilk satır) + ayırıcı + içerik + ayırıcı
        if len(lines) >= 2:
            lines.insert(1, '\n---\n')

    # ── ADIM 3: Bold yoksa → içerikteki anahtar kelimeleri bold yap ──
    if not has_bold:
        body = '\n'.join(lines[2:]) if len(lines) > 2 else '\n'.join(lines)
        # En uzun 3 kelimeyi bold yap
        words = _re.findall(r'[a-zA-ZçğıöşüÇĞİÖŞÜ]{5,}', body)
        if words:
            # Frekansa göre değil uzunluğa göre top 3
            seen = set()
            for w in sorted(set(words), key=len, reverse=True)[:3]:
                if w.lower() not in seen:
                    body = body.replace(w, f'*{w}*', 1)
                    seen.add(w.lower())
            if len(lines) > 2:
                lines = lines[:2] + [body]
            else:
    ...
    # ── ADIM 4: Kapanış yoksa → yönlendirme ekle ──
    if not has_closing:
        lines.append('\n---\n')
        lines.append('_Daha detaylı açmamı ister misin?_ 🎯')

    answer = '\n'.join(lines).strip()
    # Son temizlik: 3+ boş satır → 2
    answer = _re.sub(r'\n{4,}', '\n\n', answer)
    return answer

...
async def queue_briefings_for_window(window_start_min: int = 15,
    ...
    # 25.44 (Neo bug 12 May 18:50/18:54): log mesajı bot'u yanıltıyordu
    # Doğru ifade: altyapı hazır AMA sezon başı yetmez — Neo liste isteyince
    # test + manuel onay sonrası aktive olur. Otomatik açılış YOK.
    if not feature_active:
        logger.info(f"🎯 [TEACHER_BRIEFING] PLANLI PASIF — TEACHER_BRIEFING_ACTIVE=False "
                    f"(altyapı hazır; sezon başında Neo liste isteyecek, test+onay sonrası aktive)")
    else:
